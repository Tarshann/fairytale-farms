...
async function seedBickeringBros() {
  ...
  const existingCategory = db.all(
    sql`SELECT * FROM categories WHERE slug = 'freeze-dried-candy'`
  );
  ...
  if (existingCategory.length === 0) {
    ...
    const newCategory = db.all(
      sql`SELECT id FROM categories WHERE slug = 'freeze-dried-candy'`
    );
    ...
    console.log(
      "📦 Freeze-dried candy category already exists with ID:",
      categoryId
    );
  }
  ...
  const products = [
    {
      ...
      description:
        "Classic Skittles transformed into crunchy, airy bites with intensified fruity flavors. Taste the rainbow like never before!",
      basePrice: "8.00",
      featured: true,
    },
    {
      ...
      description:
        "Hard candies become light and puffy with an amazing crunch. The flavors are even more intense!",
      basePrice: "8.00",
      featured: true,
    },
    {
      ...
      description:
        "Chewy gummy bears transformed into crunchy, melt-in-your-mouth treats. A whole new texture experience!",
      basePrice: "7.00",
      featured: false,
    },
    {
      ...
      description:
        "Soft taffy becomes light and crispy while keeping all that delicious flavor. Simply addictive!",
      basePrice: "7.00",
      featured: false,
    },
    {
      ...
      description:
        "Sour gummy worms with an extra tangy punch! The freeze-drying process intensifies the sour coating.",
      basePrice: "8.00",
      featured: true,
    },
    {
      ...
      description:
        "Can't decide? Try them all! A mix of our most popular freeze-dried candies in one pack.",
      basePrice: "15.00",
      featured: true,
    },
  ];
  ...
  for (const product of products) {
    ...
    const existing = db.all(
      sql`SELECT * FROM products WHERE slug = ${product.slug}`
    );
