...
import { Package, Plus, Minus, ShoppingCart, Sparkles, ArrowLeft, Check, Info } from "lucide-react";
...
export default function BuildYourOwn() {
  ...
  const getItemQuantity = (productId: number) => {
    return selectedItems.find(item => item.productId === productId)?.quantity || 0;
  };
  ...
  const itemCategories = [
    ...
    { 
      name: "Pucks & Treats", 
      items: addOnItems.filter(i => i.name.toLowerCase().includes('puck') || i.name.toLowerCase().includes('brownie')),
      color: "bg-pastel-lavender",
      emoji: "🍫"
    },
    { 
      name: "Strawberries & Cakes", 
      items: addOnItems.filter(i => i.name.toLowerCase().includes('strawberry') || i.name.toLowerCase().includes('cake')),
                      ...
                      <div key={category.name}>
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                          {category.items.map((item) => {
                            ...
                            const remaining = item.inventoryCap ? item.inventoryCap - (item.inventorySold || 0) : null;
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex-1">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <span className="text-lg font-bold text-primary">${item.basePrice}</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Button
                                        ...
                                        size="icon"
                                        className="h-8 w-8"
                                        onClick={() => updateItemQuantity(item.id, item.name, parseFloat(item.basePrice), -1)}
                                        disabled={quantity === 0}
                                      >
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                    <div className="flex gap-2 text-xs text-muted-foreground">
                      ...
                      <p>Limited to 10 Build-Your-Own orders. Order by Feb 12 for Valentine's delivery.</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
