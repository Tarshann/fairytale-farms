      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 py-12">
          <div className="container text-center">
            ...
            <Link href="/my-orders"><Button>Back to Orders</Button></Link>
          </div>
        </main>
        <Footer />
      </div>
        ...
        <div className="container max-w-4xl">
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <Badge variant={order.status === 'completed' ? 'default' : 'secondary'} className="text-lg px-4 py-2">
                {order.status}
              </Badge>
            </div>
                  ...
                  {order.items?.map((item) => (
                    <div key={item.id} className="flex gap-4 pb-4 border-b border-border last:border-0 last:pb-0">
                      ...
                      <div className="text-right">
                        <p className="font-bold">${(parseFloat(item.unitPrice) * item.quantity).toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                </div>
