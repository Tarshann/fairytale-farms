...
const SITE_PAGES = [
  { id: 'valentines', name: "Valentine's Collection", path: '/valentines', description: 'Seasonal Valentine\'s Day products' },
  { id: 'custom-pucks', name: 'Custom Pucks', path: '/custom-pucks', description: 'Custom portrait pucks ordering' },
  { id: 'bickering-bros', name: 'Bickering Bros', path: '/bickering-bros', description: 'Freeze-dried candy section' },
  { id: 'gallery', name: 'Gallery', path: '/gallery', description: 'Photo gallery of past work' },
  { id: 'about', name: 'About', path: '/about', description: 'About Fairytale Farms' },
  { id: 'lab', name: 'The Lab', path: '/lab', description: 'Experimental treats section' },
];
...
export default function AdminSettings() {
  ...
  const handlePageVisibilityChange = (pageId: string, visible: boolean) => {
    ...
    localStorage.setItem('fairytale-page-visibility', JSON.stringify(newVisibility));
    toast.success(`${SITE_PAGES.find(p => p.id === pageId)?.name} ${visible ? 'shown' : 'hidden'}`);
  };
  ...
  const handlePriceUpdate = (productId: number, newPrice: string) => {
    ...
    updateProductMutation.mutate({ id: productId, basePrice: priceNum.toFixed(2) });
  };
                ...
                {SITE_PAGES.map((page) => (
                  <div key={page.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => handlePageVisibilityChange(page.id, checked)}
                    />
                  </div>
                ))}
              </div>
                  ...
                  {products.map((product) => (
                    <div key={product.id} className="flex items-center gap-4 p-4 border rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground truncate">Category ID: {product.categoryId}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Label htmlFor={`price-${product.id}`} className="sr-only">Price</Label>
                        ...
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            if (newPrice !== parseFloat(product.basePrice).toFixed(2)) {
